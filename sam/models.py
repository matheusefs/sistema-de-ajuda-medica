...
class Usuario(database.Model, UserMixin):
    id = database.Column(database.Integer, primary_key = True) #diz que é a chave primaria
...
class Medicamento(database.Model):
    id = database.Column(database.Integer, primary_key = True)
    nome = database.Column(database.String, nullable = False)
    lote = database.Column(database.Integer, nullable = False)
    forma_adm = database.Column(database.String, nullable = False) 

class Calculadora(database.Model):
    id = id = database.Column(database.Integer, primary_key = True)
    qtd_prescrita = database.Column(database.Integer, nullable = False)
    un_prescrita = database.Column(database.String, nullable = False)
    qtd_requerida = database.Column(database.Integer)
